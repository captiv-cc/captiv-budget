...
import {
  Receipt, AlertTriangle, Clock, Send, FileText,
  CheckCircle2, ChevronRight, TrendingUp, Euro,
...
const STATUTS_META = {
  en_retard:  { label: 'En retard',   icon: AlertTriangle, color: 'var(--red)',    bg: 'rgba(239,68,68,.12)'      },
  en_attente: { label: 'En attente',  icon: Clock,         color: 'var(--amber)',  bg: 'rgba(245,158,11,.12)'     },
  ...
  reglee:     { label: 'Réglée ✓',    icon: CheckCircle2,  color: 'var(--green)', bg: 'rgba(0,200,117,.12)'      },
}
...
function FactureLine({ facture }) {
  const meta = STATUTS_META[facture.statut] || STATUTS_META.brouillon
    ...
    <div className="grid items-center gap-3 px-4 py-3 text-sm transition-colors"
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {facture.client_name && (
            <span className="text-[11px]" style={{ color: 'var(--txt-3)' }}>· {facture.client_name}</span>
          )}
        </div>
      ...
      <div className="text-right">
        <p className="font-semibold text-sm" style={{ color: 'var(--txt)' }}>{fmtEur(facture.montant_ttc || 0)}</p>
        <p className="text-[11px]" style={{ color: 'var(--txt-3)' }}>{fmtEur(facture.montant_ht)} HT</p>
      </div>
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <Link to={`/projets/${facture.project_id}/factures`}
...
export default function Compta() {
  ...
  const enAttente   = factures.filter(f => ['envoyee','en_attente'].includes(f.statut)).reduce((s, f) => s + Number(f.montant_ttc || 0), 0)
  ...
  const sections = filter === 'all' ? [
    { key: 'urgent',   label: '🔴 En retard',   items: factures.filter(f => f.statut === 'en_retard')                      },
    { key: 'attente',  label: '🟡 En attente',   items: factures.filter(f => ['en_attente','envoyee'].includes(f.statut))   },
    { key: 'brouillon',label: '⚪ Brouillons',   items: factures.filter(f => f.statut === 'brouillon')                     },
    { key: 'reglee',   label: '🟢 Réglées',      items: factures.filter(f => f.statut === 'reglee')                        },
  ...
  return (
    <div className="p-6 space-y-5 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold" style={{ color: 'var(--txt)' }}>Comptabilité</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KpiCard
          ...
          icon={Euro}
          color={{ bg: 'rgba(59,130,246,.12)', txt: 'var(--blue)', activeBg: 'rgba(59,130,246,.18)' }}
        ...
        <KpiCard
          ...
          icon={Clock}
          color={{ bg: 'rgba(245,158,11,.12)', txt: 'var(--amber)', activeBg: 'rgba(245,158,11,.2)' }}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {[
          ...
          { key: 'en_retard',   label: `En retard (${factures.filter(f=>f.statut==='en_retard').length})` },
          { key: 'en_attente',  label: `En attente (${factures.filter(f=>f.statut==='en_attente').length})` },
          { key: 'envoyee',     label: `Envoyées (${factures.filter(f=>f.statut==='envoyee').length})` },
          { key: 'brouillon',   label: `Brouillons (${factures.filter(f=>f.statut==='brouillon').length})` },
          { key: 'reglee',      label: `Réglées (${factures.filter(f=>f.statut==='reglee').length})` },
        ].map(p => (
          <button key={p.key}
            ...
            style={filter === p.key
              ...
              : { background: 'var(--bg-elev)', color: 'var(--txt-3)', border: '1px solid var(--brd-sub)' }}>
            {p.label}
          </button>
        ))}
      </div>
      ...
      {/* ── Tableau ─────────────────────────────────────────────────────────── */}
      {loading ? (
        <div className="flex items-center justify-center py-20 gap-2" style={{ color: 'var(--txt-3)' }}>
        ...
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-sm" style={{ color: 'var(--txt-2)' }}>Aucune facture enregistrée</p>
          ...
          {sections.map(sec => (
            <div key={sec.key} className="rounded-xl overflow-hidden"
              ...
              <div className="flex items-center justify-between px-4 py-2.5"
                ...
                <span className="text-xs font-semibold" style={{ color: 'var(--txt-2)' }}>{sec.label}</span>
              ...
              {sec.items.map(f => <FactureLine key={f.id} facture={f} />)}
            </div>
          ))}
        </div>
      ) : (
        ...
        <div className="rounded-xl overflow-hidden"
          ...
          {displayed.length === 0
            ? <p className="text-center py-8 text-sm" style={{ color: 'var(--txt-3)' }}>Aucune facture dans cette catégorie</p>
