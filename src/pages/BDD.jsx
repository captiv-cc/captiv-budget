...
import { Plus, Search, Trash2, Edit2, Database, X, Check, ChevronDown, ChevronRight, Truck, Mail, Phone } from 'lucide-react'
...
const CAT_SUGGESTIONS = [
  'Direction','Production','Régie','Caméra','Son','Lumière','Décor','Costumes','Maquillage',
  'Post-production','Communication','Transport','Hébergement','Restauration','Location','Autre',
]

const FOURN_TYPES = ['Matériel', 'Logistique', 'Postprod', 'Catering', 'Transport', 'Hébergement', 'Autre']

const EMPTY = {
  ref: '', categorie: '', produit: '', description: '',
  unite: 'J', tarif_defaut: '', notes: '', actif: true,
}

const FOURN_EMPTY = {
  nom: '', type: '', siret: '', email: '', phone: '', notes: '',
  default_tva: 20,
}
...
export default function BDD() {
  ...
  async function loadAll() {
    setLoading(true)
    const [{ data: prods }, { data: grille }, { data: fourns }, { data: usageRows }] = await Promise.all([
      supabase.from('produits_bdd').select('*').eq('org_id', org.id).order('categorie').order('produit'),
  ...
  async function save(e) {
    ...
    if (modal === 'create') {
      ...
      if (data) setProduits(p => [...p, data].sort((a, b) =>
        (a.categorie||'').localeCompare(b.categorie||'') || a.produit.localeCompare(b.produit)
      ))
    } else {
      const { data, error } = await supabase.from('produits_bdd').update(payload).eq('id', modal.id).select().single()
      if (error) { alert(`Erreur : ${error.message}`); return }
  ...
  async function saveFourn(e) {
    ...
    if (fournModal === 'create') {
      ...
      const { data, error } = await supabase.from('fournisseurs').update(payload).eq('id', fournModal.id).select().single()
      if (error) { alert(`Erreur : ${error.message}`); return }
      if (data) setFournisseurs(p => p.map(x => x.id === data.id ? data : x).sort((a, b) => a.nom.localeCompare(b.nom)))
    }
    setFournModal(null)
  }
  ...
  async function delFourn(f) {
    ...
    setFournUsage(u => { const n = { ...u }; delete n[f.id]; return n })
  }
  ...
  const grilleByType = grilleCC.filter(g =>
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold" style={{ color: 'var(--txt)' }}>Catalogue</h1>
          <p className="text-sm" style={{ color: 'var(--txt-3)' }}>
            {totalActive} poste{totalActive !== 1 ? 's' : ''} · régime et bloc choisis à l'ajout dans le devis
          </p>

        </div>
        {tab === 'catalogue' && (
          <button onClick={() => { setForm(EMPTY); setModal('create') }} className="btn-primary btn-sm">
            <Plus className="w-3.5 h-3.5" /> Nouveau poste
          </button>
        )}
        {tab === 'fournisseurs' && (
          <button onClick={() => { setFournForm(FOURN_EMPTY); setFournModal('create') }} className="btn-primary btn-sm">
      ...
      {/* Tabs */}
      <div className="flex gap-1 mb-5 p-1 rounded-xl w-fit" style={{ background: 'var(--bg-elev)' }}>
        {[['catalogue', 'Postes & prestations'], ['fournisseurs', 'Fournisseurs'], ['grille', 'Grille CC Audiovisuelle']].map(([val, lbl]) => (
          <button key={val} onClick={() => setTab(val)}
            className="px-4 py-1.5 rounded-lg text-sm font-medium transition-all"
            style={tab === val
              ? { background: 'var(--bg-surf)', color: 'var(--txt)', boxShadow: '0 1px 4px rgba(0,0,0,.3)' }
              : { color: 'var(--txt-3)' }}
          >
            {lbl}
          </button>
      ...
      <div className="relative mb-5">
        ...
        <input className="input pl-9 w-full" value={search}
          onChange={e => setSearch(e.target.value)} placeholder="Rechercher un poste, une prestation…" />
      </div>
      ...
      {tab === 'catalogue' && (
        <div className="space-y-3">
          ...
          {!loading && filtered.length === 0 && (
            <div className="rounded-xl p-16 text-center" style={{ border: '1px dashed var(--brd)' }}>
          ...
          {cats.map(cat => {
            const lines = [...(byCat[cat] || [])].sort((a, b) =>
              (b.actif ? 1 : 0) - (a.actif ? 1 : 0) || a.produit.localeCompare(b.produit)
            )
            const isCollapsed = collapsed[cat]

            return (
              <div key={cat || '__sans_cat'} className="rounded-xl overflow-hidden"
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-[11px] font-semibold uppercase"
                        ...
                        <th className="px-4 py-2 text-center w-10" title="Actif">✓</th>
                      ...
                      {lines.map(line => (
                        ...
                        >
                          <td className="px-4 py-2.5">
                            <p className="font-medium text-sm" style={{ color: 'var(--txt)' }}>{line.produit}</p>
                            {line.ref && (
                              <p className="text-[11px]" style={{ color: 'var(--txt-3)' }}>#{line.ref}</p>
                            )}
                          </td>
                          <td className="px-4 py-2.5" style={{ maxWidth: 260 }}>
                            {line.description
                              ? <p className="text-xs" style={{ color: 'var(--txt-2)' }}>{line.description}</p>
                              : <span className="text-xs" style={{ color: 'var(--txt-3)' }}>—</span>
                            }
                            {line.notes && (
                              <p className="text-[11px] italic mt-0.5" style={{ color: 'var(--txt-3)' }}>{line.notes}</p>
                            )}
                          </td>
                          <td className="px-4 py-2.5 text-center text-xs" style={{ color: 'var(--txt-3)' }}>
                            {line.unite}
                          </td>
                          ...
                          <td className="px-4 py-2.5 text-center">
                            <button onClick={() => toggleActif(line)}
                          ...
                          <td className="px-4 py-2.5">
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => {
                                setForm({ ...EMPTY, ...line, tarif_defaut: line.tarif_defaut ?? '' })
                                setModal(line)
                              }} className="btn-ghost btn-sm">
                                <Edit2 className="w-3.5 h-3.5" />
                              </button>
                              <button onClick={() => del(line.id)} className="btn-ghost btn-sm"
                                style={{ color: 'var(--txt-3)' }}
      ...
      {tab === 'fournisseurs' && (() => {
        ...
        const fFiltered = fournisseurs.filter(f =>
          !fq ||
        ...
        return (
          <div className="space-y-3">
            ...
            {types.map(type => {
              ...
              return (
                <div key={type || '__sans_type'} className="rounded-xl overflow-hidden"
                  style={{ border: '1px solid var(--brd)' }}>
                  <div className="flex items-center gap-3 px-4 py-3"
                    style={{ background: 'var(--bg-elev)', borderLeft: '4px solid var(--purple)' }}>
                    <span className="text-xs font-bold uppercase tracking-wider flex-1"
                    ...
                    <tbody>
                      {items.map(f => {
                        return (
                          ...
                          >
                            <td className="px-4 py-2.5">
                              <p className="font-medium text-sm" style={{ color: 'var(--txt)' }}>{f.nom}</p>
                              {f.notes && (
                                <p className="text-[11px] italic mt-0.5" style={{ color: 'var(--txt-3)' }}>{f.notes}</p>
                              )}
                            </td>
                            <td className="px-4 py-2.5">
                              {f.email && (
                              ...
                              {f.phone && (
                                <div className="flex items-center gap-1.5 text-xs mt-0.5" style={{ color: 'var(--txt-2)' }}>
                            ...
                            <td className="px-4 py-2.5">
                              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <button onClick={() => delFourn(f)} className="btn-ghost btn-sm"
                                  style={{ color: 'var(--txt-3)' }}
                                  onMouseEnter={e => e.currentTarget.style.color = 'var(--red)'}
                                  onMouseLeave={e => e.currentTarget.style.color = 'var(--txt-3)'}
                                >
      ...
      {tab === 'grille' && (
        <div className="space-y-4">
          {Object.keys(grilleByType).length === 0 && (
            <p className="text-sm text-center py-10" style={{ color: 'var(--txt-3)' }}>Aucune donnée</p>
          )}
          {Object.entries(grilleByType).map(([type, items]) => (
            <div key={type} className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--brd)' }}>
              <div className="px-4 py-2.5 text-xs font-bold uppercase tracking-wider"
                style={{ background: 'var(--bg-elev)', color: 'var(--txt-2)', borderBottom: '1px solid var(--brd-sub)' }}>
                {type}
              </div>
              <table className="w-full text-sm">
                <thead>
                ...
                <tbody>
                  {items.map(g => (
                    <tr key={g.id} style={{ borderBottom: '1px solid var(--brd-sub)' }}>
                      <td className="px-4 py-2 font-medium" style={{ color: 'var(--txt)' }}>{g.intitule}</td>
                      <td className="px-4 py-2 text-center text-xs" style={{ color: 'var(--txt-3)' }}>{g.filiere || '—'}</td>
                      <td className="px-4 py-2 text-center text-xs" style={{ color: 'var(--txt-3)' }}>{g.niveau || '—'}</td>
                      <td className="px-4 py-2 text-right font-semibold" style={{ color: 'var(--green)' }}>
                        {g.journee_min ? fmtEur(g.journee_min) : (
                          <span className="text-xs italic" style={{ color: 'var(--txt-3)' }}>Gré à gré</span>
                        )}
                      </td>
                      <td className="px-4 py-2 text-right text-xs" style={{ color: 'var(--txt-3)' }}>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center"
          ...
          <div className="rounded-2xl w-full max-w-md"
            style={{ background: 'var(--bg-surf)', border: '1px solid var(--brd)', boxShadow: '0 24px 80px rgba(0,0,0,.8)' }}>
            ...
            <form onSubmit={save} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input className="input w-full" required autoFocus
                  value={form.produit}
              ...
              <div>
                <label className="label">
                  Sous-catégorie
                  <span className="ml-1 font-normal" style={{ color: 'var(--txt-3)' }}>(pour regrouper)</span>
                </label>
                <input className="input w-full" list="cat-suggestions"
              ...
              <div className="flex gap-3">
                <div className="w-28">
                  ...
                  <select className="input w-full" value={form.unite}
                    onChange={e => setForm(f => ({ ...f, unite: e.target.value }))}>
                    {UNITES.map(u => <option key={u} value={u}>{u}</option>)}
                  </select>
                </div>
                <div className="flex-1">
                  <label className="label">Tarif indicatif HT (€) <span className="font-normal" style={{ color: 'var(--txt-3)' }}>(optionnel)</span></label>
              ...
              {/* Notes */}
              <div>
                <label className="label">Notes <span className="font-normal" style={{ color: 'var(--txt-3)' }}>(fournisseur, conditions…)</span></label>
      ...
      {fournModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center"
          style={{ background: 'rgba(0,0,0,.7)', backdropFilter: 'blur(4px)' }}
          onClick={e => { if (e.target === e.currentTarget) setFournModal(null) }}>
          <div className="rounded-2xl w-full max-w-md"
            style={{ background: 'var(--bg-surf)', border: '1px solid var(--brd)', boxShadow: '0 24px 80px rgba(0,0,0,.8)' }}>
            ...
            <form onSubmit={saveFourn} className="p-6 space-y-4">
              ...
              <div>
                <label className="label">Type <span className="font-normal" style={{ color: 'var(--txt-3)' }}>(optionnel)</span></label>
                <input className="input w-full" list="fourn-types"
                ...
                <datalist id="fourn-types">
                  {FOURN_TYPES.map(t => <option key={t} value={t} />)}
                </datalist>
              </div>
              ...
              <div>
                <label className="label">SIRET <span className="font-normal" style={{ color: 'var(--txt-3)' }}>(optionnel)</span></label>
                <input className="input w-full"
              ...
              <div className="flex justify-end gap-2 pt-1">
                <button type="button" onClick={() => setFournModal(null)} className="btn-secondary">Annuler</button>
                <button type="submit" className="btn-primary">
                  <Check className="w-4 h-4" /> Enregistrer
                </button>
              </div>
