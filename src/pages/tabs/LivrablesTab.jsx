    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-8">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center"
          style={{ background: 'rgba(0,200,117,.12)' }}>
          <CheckSquare className="w-5 h-5" style={{ color: 'var(--green)' }} />
        </div>
        <div>
          <h1 className="text-lg font-bold" style={{ color: 'var(--txt)' }}>Livrables</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="rounded-xl p-5" style={{ background: 'var(--bg-surf)', border: '1px solid var(--brd)' }}>
          <div className="flex items-center gap-2 mb-4">
            <Package className="w-4 h-4" style={{ color: 'var(--green)' }} />
            <h3 className="font-semibold text-sm" style={{ color: 'var(--txt)' }}>Fonctionnalités à venir</h3>
          </div>
          <ul className="space-y-2">
            {FEATURES.map((f, i) => (
              <li key={i} className="flex items-start gap-2 text-xs" style={{ color: 'var(--txt-2)' }}>
                <CheckCircle2 className="w-3.5 h-3.5 shrink-0 mt-0.5" style={{ color: 'var(--green)', opacity: 0.7 }} />
                {f}
              </li>
            ))}
          </ul>
        ...
        <div className="rounded-xl p-5" style={{ background: 'var(--bg-surf)', border: '1px solid var(--brd)' }}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-sm" style={{ color: 'var(--txt)' }}>Cycle de révision</h3>
          </div>
        ...
        <div className="md:col-span-2 rounded-xl p-5" style={{ background: 'var(--bg-surf)', border: '1px solid var(--brd)' }}>
          <h3 className="font-semibold text-sm mb-4" style={{ color: 'var(--txt)' }}>
            Aperçu — tableau des livrables
          </h3>
          <div className="rounded-lg overflow-hidden" style={{ border: '1px solid var(--brd-sub)', opacity: 0.6 }}>
            {/* Header */}
            ...
            {EXEMPLE_LIVRABLES.map((l, i) => (
              <div key={i}
                ...
                <span className="font-medium" style={{ color: 'var(--txt)' }}>{l.nom}</span>
                <span>{l.format}</span>
                <span>{l.duree}</span>
                <span>0 / 2</span>
