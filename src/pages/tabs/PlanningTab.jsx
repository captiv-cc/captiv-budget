import { Calendar, Clock, Film, Map, FileText, Mail, CheckSquare } from 'lucide-react'
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-lg font-bold" style={{ color: 'var(--txt)' }}>Planning</h1>
    ...
    <div className="rounded-xl p-5" style={{ background: 'var(--bg-surf)', border: '1px solid var(--brd)' }}>
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="font-semibold text-sm" style={{ color: 'var(--txt)' }}>{title}</h3>
        </div>
        {comingSoon && (
      ...
      <p className="text-xs mb-4" style={{ color: 'var(--txt-3)' }}>{description}</p>
      <ul className="space-y-2">
        {features.map((f, i) => (
          <li key={i} className="flex items-start gap-2 text-xs" style={{ color: 'var(--txt-2)' }}>
            <CheckSquare className="w-3.5 h-3.5 shrink-0 mt-0.5" style={{ color: c.fg, opacity: 0.7 }} />
            {f}
          </li>
        ))}
      </ul>
