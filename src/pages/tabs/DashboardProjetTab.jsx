...
import {
  TrendingUp, TrendingDown, Euro, Users, Receipt, BarChart3,
  CheckCircle2, Clock, AlertTriangle, ArrowRight, Minus,
  FileText, Activity, ShieldCheck, Percent,
} from 'lucide-react'
...
function pctBar(val, max, color) {
  ...
  return (
    <div className="h-1.5 rounded-full overflow-hidden mt-1" style={{ background: 'var(--bg-elev)' }}>
...
function KpiCard({ label, value, sub, icon: Icon, color, trend, to }) {
  const inner = (
    <div className="rounded-xl p-4 transition-all h-full"
      style={{
        background: 'var(--bg-surf)',
        border: '1px solid var(--brd)',
      }}
      onMouseEnter={e => { if (to) e.currentTarget.style.borderColor = color?.txt || 'var(--brd)' }}
      onMouseLeave={e => { if (to) e.currentTarget.style.borderColor = 'var(--brd)' }}>
      ...
      <p className="text-[11px] uppercase tracking-wide mb-0.5" style={{ color: 'var(--txt-3)' }}>{label}</p>
      <p className="text-xl font-bold leading-tight" style={{ color: 'var(--txt)' }}>{value}</p>
  ...
  return (
    <div className="flex items-center justify-between mb-3">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-xs font-bold uppercase tracking-widest" style={{ color: 'var(--txt-3)' }}>{title}</h2>
      </div>
      {action}
    </div>
  )
...
export default function DashboardProjetTab() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('projet_membres').select('*, contact:contacts(nom, prenom, specialite)').eq('project_id', projectId),
    ]).then(([br, fac, mb]) => {
  ...
  if (!refSynth) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3" style={{ color: 'var(--txt-3)' }}>
        <BarChart3 className="w-10 h-10 opacity-30" />
        <p className="text-sm">Créez un devis pour voir le dashboard</p>
        <Link to={`/projets/${projectId}/devis`}
  ...
  const totalFacHT   = factures.reduce((s, f) => s + Number(f.montant_ht || 0), 0)
  ...
  const facAttente   = factures.filter(f => ['envoyee','en_attente'].includes(f.statut))

  // Marge couleur
  const margeColor = refSynth.pctMargeFinale < 0 ? 'var(--red)'
    : refSynth.pctMargeFinale > 0.20 ? 'var(--green)' : 'var(--amber)'
    ...
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <KpiCard
          ...
          icon={TrendingUp}
          color={{ bg: refSynth.pctMargeFinale < 0 ? 'rgba(239,68,68,.12)' : 'rgba(0,200,117,.12)', txt: margeColor }}
        />
        <KpiCard
          label="Budget réel"
          value={fmtEur(totalReel)}
          sub={totalReel > 0 ? `Écart : ${ecartBudget >= 0 ? '+' : ''}${fmtEur(ecartBudget)}` : 'Aucune entrée'}
          icon={Activity}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl p-5"
          ...
          <SectionHeader icon={Activity} title="Budget vs Réel"
            action={
              <Link to={`/projets/${projectId}/budget`}
          ...
          {budgetReel.length === 0 ? (
            <div className="flex flex-col items-center py-8 gap-2" style={{ color: 'var(--txt-3)' }}>
            ...
            <div className="space-y-4">
              ...
              {/* Écart résumé */}
              <div className="rounded-lg p-3 flex items-center justify-between mt-2"
                style={{
                ...
                <span className="text-sm font-bold"
                  ...
                  {ecartBudget >= 0 ? '+' : ''}{fmtEur(ecartBudget)}
                </span>
              </div>
            </div>
          )}
        ...
        <div className="rounded-xl p-5"
          style={{ background: 'var(--bg-surf)', border: '1px solid var(--brd)' }}>
          <SectionHeader icon={Receipt} title="Facturation"
            action={
              <Link to={`/projets/${projectId}/factures`}
                className="flex items-center gap-1 text-[11px] transition-colors"
                style={{ color: 'var(--txt-3)' }}
                onMouseEnter={e => e.currentTarget.style.color = 'var(--blue)'}
                onMouseLeave={e => e.currentTarget.style.color = 'var(--txt-3)'}>
          ...
          {factures.length === 0 ? (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="h-3 rounded-full overflow-hidden" style={{ background: 'var(--bg-elev)' }}>
                  <div className="h-full rounded-full transition-all duration-700"
                    style={{
              ...
              {prochainesFac.length > 0 && (
                <div className="space-y-1.5">
                  <p className="text-[10px] font-bold uppercase tracking-wider" style={{ color: 'var(--txt-3)' }}>
                    À encaisser
                  </p>
                  {prochainesFac.map(f => (
                    <div key={f.id}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-2 rounded-xl overflow-hidden"
          ...
          <div className="flex items-center justify-between px-5 py-3"
            ...
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xs font-bold uppercase tracking-widest" style={{ color: 'var(--txt-3)' }}>
                Versions devis
              </h2>
            </div>
            <Link to={`/projets/${projectId}/devis`}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr style={{ background: 'var(--bg-elev)', borderBottom: '1px solid var(--brd-sub)' }}>
              ...
              <tbody>
                {devisList.map(dv => {
                  ...
                  const mc = !s ? '' : s.pctMargeFinale < 0 ? 'var(--red)' : s.pctMargeFinale > 0.2 ? 'var(--green)' : 'var(--amber)'
                  const statusMap = {
                  ...
                  return (
                    <tr key={dv.id}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <Link to={`/projets/${projectId}/devis/${dv.id}`}
                            className="font-bold hover:underline" style={{ color: 'var(--txt)' }}>
                            V{dv.version_number}
                          </Link>
                          {isRef && (
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl p-5"
            style={{ background: 'var(--bg-surf)', border: '1px solid var(--brd)' }}>
            <SectionHeader icon={Users} title="Équipe"
              action={
                <Link to={`/projets/${projectId}/equipe`}
                  className="flex items-center gap-1 text-[11px] transition-colors"
                  style={{ color: 'var(--txt-3)' }}
                  onMouseEnter={e => e.currentTarget.style.color = 'var(--blue)'}
                  onMouseLeave={e => e.currentTarget.style.color = 'var(--txt-3)'}>
            ...
            />
            {membres.length === 0 ? (
              <p className="text-xs text-center py-4" style={{ color: 'var(--txt-3)' }}>Aucun membre</p>
            ) : (
              <div className="space-y-1.5">
                {membres.slice(0, 6).map(m => {
                  ...
                  return (
                    <div key={m.id} className="flex items-center gap-2 rounded-lg px-2 py-1.5"
                      ...
                      <div className="min-w-0">
                        <p className="text-xs font-medium truncate" style={{ color: 'var(--txt)' }}>{nom}</p>
                        {spec && <p className="text-[10px] truncate" style={{ color: 'var(--txt-3)' }}>{spec}</p>}
                      </div>
                    </div>
                  )
                })}
          ...
          <div className="rounded-xl p-5"
            ...
            <div className="space-y-2">
              {[
                ...
                refSynth.montantMargeGlobale ? { label: `Mg+Fg ${refDevis?.marge_globale_pct || 0}%`, val: refSynth.montantMargeGlobale, color: 'var(--blue)' } : null,
                refSynth.montantAssurance    ? { label: `Assurance ${refDevis?.assurance_pct || 0}%`, val: refSynth.montantAssurance, color: '#a78bfa' } : null,
                refSynth.totalCharges        ? { label: 'Charges soc. pat.', val: refSynth.totalCharges, color: 'var(--red)' } : null,
                refSynth.montantRemiseGlobale? { label: 'Remise', val: -refSynth.montantRemiseGlobale, color: 'var(--orange)' } : null,
                { label: 'Total HT final', val: refSynth.totalHTFinal, color: 'var(--txt)', bold: true },
              ].filter(Boolean).map((row, i) => (
                <div key={i} className="flex items-center justify-between">
                  <span className="text-[11px]" style={{ color: row.color, fontWeight: row.bold ? 700 : 400 }}>
                    {row.label}
                  </span>
