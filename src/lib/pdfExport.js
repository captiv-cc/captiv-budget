...
export async function exportDevisPDF(devis, project, client, org, taux = TAUX_DEFAUT) {
  ...
  doc.addFileToVFS('WorkSans-Regular.ttf', wsRegB64);  doc.addFont('WorkSans-Regular.ttf', 'WS', 'normal')
  doc.addFileToVFS('WorkSans-Bold.ttf',    wsBoldB64); doc.addFont('WorkSans-Bold.ttf',    'WS', 'bold')
  doc.addFileToVFS('WorkSans-Medium.ttf',  wsMedB64);  doc.addFont('WorkSans-Medium.ttf',  'WS', 'medium')
  ...
  const synth       = calcSynthese(activeLines, devis.tva_rate || 20, devis.acompte_pct || 30, taux, globalAdj)
  ...
  const drawDetailHeader   = () => {
    ...
    txt(`${NUM} | PROJET : ${project?.title || '—'}`, PW / 2, 11, { size: 7.5, bold: true, align: 'center' })
  ...
  ly += 3.8
  if (org?.address) {
    doc.splitTextToSize(org.address, halfW).forEach(l => { txt(l, M, ly, { size: 6.5, color: C.gray }); ly += 3.4 })
  } else {
    txt('59 Rue de Ponthieu, Bureau 326', M, ly, { size: 6.5, color: C.gray }); ly += 3.4
    txt('75008 PARIS',                   M, ly, { size: 6.5, color: C.gray }); ly += 3.4
    txt('FRANCE',                        M, ly, { size: 6.5, color: C.gray }); ly += 3.4
  }
  ly += 0.5
  txt(org?.contact_name || 'Hugo MARTIN',    M, ly, { size: 6.5, bold: true, color: C.gray }); ly += 3.4
  txt(org?.email        || 'hugo@captiv.cc', M, ly, { size: 6.5, color: C.gray });              ly += 3.4
  txt(org?.phone        || '0641675554',     M, ly, { size: 6.5, color: C.gray });              ly += 3.4

  // Client (droite, sur fond gris déjà dessiné) — démarre 4mm plus bas (ligne vide en haut)
  let cy = y + 4
  if (client?.name)         { txt(client.name,         rightX, cy, { size: 7.5, bold: true }); cy += 3.8 }
  if (client?.contact_name) { txt(client.contact_name, rightX, cy, { size: 6.5, color: C.gray }); cy += 3.4 }
  if (client?.address) {
    doc.splitTextToSize(client.address, halfW).forEach(l => { txt(l, rightX, cy, { size: 6.5, color: C.gray }); cy += 3.4 })
  }
  if (client?.email) { txt(client.email, rightX, cy, { size: 6.5, color: C.gray }); cy += 3.4 }
  ...
  const c1W  = 57,  c2W  = 57,  c3W  = IW - c1W - c2W - 8
  const c1X  = M,   c2X  = M + c1W + 4,  c3X = c2X + c2W + 4
  ...
  let p3y = y
  txt('Livrable(s) :', c3X, p3y, { size: 6, color: C.gray }); p3y += 3.4
  ...
  if (livsJson.length > 0) {
    livsJson.slice(0, 8).forEach((liv, i) => {
      ...
      doc.splitTextToSize(parts.join(' | '), c3W).forEach(l => {
        txt(l, c3X, p3y, { size: 6, color: C.gray }); p3y += 3.1
      })
    })
  } else {
    txt('TBD', c3X, p3y, { size: 6, color: C.gray }); p3y += 3.1
  }
  ...
  txt('Sous-totaux HT', PW - M - 2, y + 3.4, { size: 6.5, bold: true, color: C.white, align: 'right' })
  y += RECAP_HDR_H
  ...
  if (synth.montantRemiseGlobale > 0) {
    ...
    txt(`- ${fmtEur(synth.montantRemiseGlobale)}`, PW - M - 2, y + 3.5, { size: 6.5, align: 'right' })
    y += ST_ROW_H
  }
  ...
  if (project?.note_prod) {
    txt('HORS DEVIS / NOTE DE PROD', M, ny, { size: 6, bold: true }); ny += 3.5
    doc.splitTextToSize(project.note_prod, notesW).forEach(l => {
      txt(l, M, ny, { size: 5.5, color: C.gray }); ny += 2.8
    })
    ny += 2
  }

  txt('ANNULATION / REPORT', M, ny, { size: 6, bold: true }); ny += 3.5
  ;[
  ...
  ].forEach(t => doc.splitTextToSize(t, notesW).forEach(l => { txt(l, M, ny, { size: 5.5, color: C.gray }); ny += 2.8 }))
  ny += 2

  txt('MODALITÉS DE RÈGLEMENT', M, ny, { size: 6, bold: true }); ny += 3.5
  ;[
  ...
  txt('CGV', M, ny, { size: 6, bold: true }); ny += 3.5
  doc.splitTextToSize(
    ...
    notesW
  ).forEach(l => { txt(l, M, ny, { size: 5.5, color: C.gray }); ny += 2.8 })
  ...
  txt(fmtEur(synth.totalHTFinal), totsX + totsW - 2, ty + 4.3, { size: 8, bold: true, color: C.white, align: 'right' })
  ty += RH + 1
  ...
  txt(fmtEur(synth.totalTTC), totsX + totsW - 2, ty + 3.8, { size: 6.5, bold: true, align: 'right' })
  ty += RH
  ...
  txt('Offre valable jusqu\'au', totsX + totsW / 2, ty + 2.5, { size: 5.5, color: C.gray, align: 'center' })
  ...
  txt('BON POUR ACCORD ET SIGNATURE', totsX + totsW / 2, ty + 4.5, { size: 6.5, bold: true, align: 'center' }); ty += 10
  txt('Fait à :',    totsX + 3, ty, { size: 6.5, color: C.gray }); ty += 9
  txt('Le :',        totsX + 3, ty, { size: 6.5, color: C.gray }); ty += 9
  ...
  for (const cat of categories) {
    ...
    for (const line of catLines) {
      ...
      body.push([
        ...
        { content: '', _produit: line.produit || '', _ref: line.ref || '', _descr: line.description || '' },
        { content: qtDisp,                                          styles: { halign: 'right',  fontSize: 6.5, cellPadding: colPad } },
        { content: line.unite || '',                                styles: { halign: 'center', fontSize: 6.5, cellPadding: colPad } },
        { content: line.tarif_ht ? fmtEur(line.tarif_ht) : '',    styles: { halign: 'right',  fontSize: 6.5, cellPadding: colPad } },
        { content: remise,                                          styles: { halign: 'center', fontSize: 6.5, textColor: C.gray, cellPadding: colPad } },
        { content: fmtEur(c.prixVenteHT),                          styles: { halign: 'right',  fontSize: 6.5, fontStyle: 'bold', cellPadding: colPad } },
      ])
    }
    ...
    body.push([
      {
        ...
        colSpan: 5,
        styles: {
          textColor: C.gray, fontSize: 6, halign: 'right', font: 'WS',
      ...
      },
      {
        ...
        styles: {
          textColor: C.black, fontSize: 7.5, fontStyle: 'bold', halign: 'right', font: 'WS',
    ...
    const hdrCell = (content, align = 'right') => ({
      content,
      styles: {
        fillColor: C.header, textColor: C.white, fontSize: 6.5,
        fontStyle: 'normal', halign: align, font: 'WS',
        cellPadding: { left: 1.5, top: 2, bottom: 2, right: 1.5 },
      },
    })
    ...
    autoTable(doc, {
      startY: dy,
      head: [[
        { content: catDisplayName, styles: { fillColor: C.header, textColor: C.white, fontSize: 7, fontStyle: 'bold', halign: 'left', font: 'WS', cellPadding: colPadProd } },
        hdrCell('QT'),
      ...
      didDrawCell(data) {
        ...
        if (_descr) {
          ...
          doc.text(_descr, data.cell.x + data.cell.width - 2, midY, {
            align: 'right', maxWidth: descrMaxW,
          })
        }
      },

      didDrawPage() { drawDetailHeader() },
    })
  ...
  ].forEach(l => { txt(l, PW / 2, legalY, { size: 6, color: C.gray, align: 'center' }); legalY += 3.5 })

  // ── Footers ───────────────────────────────────────────────────────────────────
  const totalPages = doc.internal.getNumberOfPages()
  for (let i = 1; i <= totalPages; i++) { doc.setPage(i); drawFooter(i) }

  // ── Save ──────────────────────────────────────────────────────────────────────
  const slug     = (project?.ref_projet || project?.title || 'CAPTIV').replace(/[^a-zA-Z0-9-]/g, '_').slice(0, 30)
