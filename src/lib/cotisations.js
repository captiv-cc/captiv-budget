...
export const TYPES_PROJET = [
  'Film institutionnel', 'Publicité', 'Clip musical', 'Documentaire',
  'Court-métrage', 'Long-métrage', 'Reportage', 'Émission TV',
  'Captation événement', 'Formation', 'Autre'
]
...
export const CATS_HUMAINS = ['Intermittent Technicien', 'Intermittent Artiste', 'Ext. Intermittent', 'Interne', 'Externe']
...
export function calcLine(line, taux = TAUX_DEFAUT) {
  ...
  const prixVenteHT = line.use_line
    ? qt * tarif * (1 - remise / 100)
    : 0
 ...
 */
export function calcSynthese(lines, tvaTaux = 20, acomptePct = 30, taux = TAUX_DEFAUT, global = {}) {
  ...
  const montantRemiseGlobale  = remiseGlobaleMontant > 0
...
export const fmtEur = (v) =>
  Number(v || 0).toLocaleString('fr-FR', { style:'currency', currency:'EUR', minimumFractionDigits:2 })
