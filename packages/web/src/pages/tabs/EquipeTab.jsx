...
function regimeStyle(regime) {
  if (!regime)
    return { bg: 'var(--bg-elev)', color: 'var(--txt-3)', border: '1px solid var(--brd-sub)' }
  const r = regime.toLowerCase()
  if (r.includes('intermittent'))
    return {
      bg: 'rgba(156,95,253,.14)',
      color: 'var(--purple)',
      border: '1px solid rgba(156,95,253,.3)',
    }
  if (r === 'interne')
    return {
...
export default function EquipeTab() {
    ...
    try {
      ...
      const promises = [
        supabase
          .from('projet_membres')
          .select(
            '*, contact:contacts(nom, prenom, email, telephone, specialite, tarif_jour_ref, user_id)',
          )
  ...
  function getMembre(line) {
    return (
      membres.find((m) => m.devis_line_id === line.id) ||
      membres.find(
        (m) =>
          !m.devis_line_id && m.specialite?.toLowerCase() === (line.produit || '').toLowerCase(),
      )
    )
  }
  ...
  async function addMembre(line, contactOrData) {
    ...
    if (!error && data) {
      setMembres((p) => [...p, data])
      showToast(`${fullName(data)} attribué·e`)
    }
  }
  ...
  async function saveToBDD(membre) {
    ...
    const { data: ct, error } = await supabase
      ...
      .insert({
        ...
        regime: membre.regime || 'Externe',
        specialite: membre.specialite || null,
        tarif_jour_ref: membre.tarif_jour || null,
        actif: true,
      })
      .select()
      .single()
  ...
  function exportCSV() {
    ...
    // produire un download invisible. Le BOM U+FEFF en tête garantit
    // qu'Excel ouvre l'UTF-8 sans casser les accents.
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      {toast && (
        <div
          className="fixed bottom-6 right-6 z-50 px-4 py-2.5 rounded-xl shadow-lg text-sm font-medium flex items-center gap-2"
          style={{
...
function PosteCard({
  ...
  const borderColor =
    statut === 'contrat_signe' || statut === 'paie_terminee' || statut === 'paie_en_cours'
      ? 'rgba(0,200,117,.35)'
      : statut === 'integre'
        ? 'rgba(156,95,253,.25)'
        : 'var(--brd)'
...
function PersonSearch({ onAdd, _roleHint, regime }) {
  const { org } = useAuth()
          ...
          {mode === 'bdd' ? (
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs" style={{ color: 'var(--txt-3)' }}>
                        {[
                          c.specialite,
                          c.regime,
                          c.tarif_jour_ref != null ? fmtEur(c.tarif_jour_ref) + '/j' : null,
                        ]
                          .filter(Boolean)
                          .join(' · ')}
                      </p>
            ...
            <form onSubmit={submitLibre} className="p-4 space-y-2.5">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[10px] mb-1 block" style={{ color: 'var(--txt-3)' }}>
                    Prénom
                  </label>
                  <input
                    ...
                    value={libre.prenom}
                    autoFocus
                    onChange={(e) => setLibre((f) => ({ ...f, prenom: e.target.value }))}
                    placeholder="Jean"
                  />
                </div>
                <div>
                  ...
                  <input
                    className="input text-sm h-9 w-full"
                    value={libre.nom}
                    onChange={(e) => setLibre((f) => ({ ...f, nom: e.target.value }))}
                    placeholder="Dupont"
                  />
                </div>
              </div>
    ...
    <>
      ...
      >
        <Search
          ...
          style={{ color: open ? 'var(--blue)' : 'var(--txt-3)' }}
        />
          ...
          >
            {STEPS.map((s, i) => (
              <button
                key={s.key}
                onClick={() => {
                  onUpdate(membreId, { movinmotion_statut: s.key })
                  setOpen(false)
                }}
              ...
              >
                ...
                <span
                  style={{
                    color: s.key === statut ? s.color : 'var(--txt-2)',
                    fontWeight: s.key === statut ? 700 : 400,
                  }}
                >
                  {s.label}
                </span>
                {s.key === statut && (
                  <Check className="w-3 h-3 ml-auto" style={{ color: s.color }} />
                )}
              </button>
            ))}
          </div>,
...
function EquipeFinancesView({
  ...
  const totalConvenu = filteredMembres.reduce((s, m) => {
    ...
    return (
      s +
      (REGIMES_SALARIES.includes(line.regime) || line.regime === 'Ext. Intermittent'
        ? c.coutReelHT
        : c.coutCharge)
    )
  }, 0)
    ...
    <div className="space-y-4">
      ...
      {(canSeeFinance || canSeeCrewBudget) && (
        <div
          ...
          style={{
            gridTemplateColumns: `repeat(${canSeeFinance ? (canSeeCrewBudget ? 3 : 2) : 1}, 1fr)`,
          }}
        >
          {canSeeFinance && (
            <KpiCard
...
function PersonCard({
  ...
  // Calculs agrégés
  const totalVente = person.postes.reduce(
    (s, { line }) => s + (line ? calcLine(line).prixVenteHT : 0),
    0,
  )
  const totalCout = person.postes.reduce(
    (s, { line }) => s + (line ? calcLine(line).coutCharge : 0),
    0,
  )
  ...
  const stepIdxMin = Math.min(
    ...person.postes.map(
      ({ membre: m }) => STEPS.findIndex((s) => s.key === m.movinmotion_statut) || 0,
    ),
  )
  const globalStep = STEPS[Math.max(0, stepIdxMin)]
  const borderColor =
    globalStep.key === 'contrat_signe' || globalStep.key === 'paie_terminee'
      ? 'rgba(0,200,117,.35)'
      : globalStep.key === 'integre'
        ? 'rgba(156,95,253,.25)'
        : 'var(--brd)'
          ...
          {canSeeCrewBudget && (
            <div className="flex-1 px-4 py-2.5">
              ...
              <p
                ...
                style={{ color: hasCustomBudget ? 'var(--blue)' : 'var(--txt-3)' }}
              >
                {fmtEur(totalBudget)}
              </p>
              {!hasCustomBudget && (
...
function PosteFinanceRow({
  ...
  }, [m.id, m.budget_convenu, budgetRef])
  ...
  const budgetAlert =
    m.budget_convenu != null && calc
      ? m.budget_convenu > calc.prixVenteHT
        ? '⚠ dépasse la vente HT'
        : m.budget_convenu < budgetRef
          ? '⚠ sous le coût estimé'
          : null
      : null
    ...
    <div className="px-4 py-3 space-y-2">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          {bloc && (
            <p
              className="text-[10px] font-semibold uppercase tracking-wide leading-none mb-0.5 truncate"
              style={{ color: bloc.color }}
            >
              {bloc.label}
            </p>
        ...
        {rs && (
          <span
            className="text-[10px] px-2 py-0.5 rounded-full font-semibold shrink-0 hidden sm:inline"
            style={{ background: rs.bg, color: rs.color, border: rs.border }}
          >
            {line.regime}
          </span>
        ...
        <div className="flex items-start gap-4 pl-1 flex-wrap">
          ...
          {canSeeFinance && (
            <div className="text-xs">
              <p className="text-[10px] uppercase tracking-wide" style={{ color: 'var(--txt-3)' }}>
                Vente HT
              </p>
              <p className="font-semibold" style={{ color: 'var(--txt)' }}>
                {fmtEur(calc.prixVenteHT)}
              </p>
            ...
            <div className="text-xs">
              {isIntermittent ? (
                <>
                  ...
                  <p className="font-semibold" style={{ color: 'var(--purple)' }}>
                    {fmtEur(calc.coutReelHT)}
                  </p>
                  <p className="text-[10px]" style={{ color: 'var(--txt-3)' }}>
                    + {fmtEur(calc.chargesPat)} →{' '}
                    <span style={{ color: 'var(--amber)' }}>{fmtEur(calc.coutCharge)}</span>
                  </p>
                </>
              ) : (
                <>
              ...
              {editBudget ? (
                <div className="flex items-center gap-1">
                  <input
                    type="number"
                    autoFocus
                    className="input text-xs h-6 w-24"
                    value={budgetVal}
          ...
          {isIntermittent && (
            <div className="flex items-center gap-1 ml-auto self-center">
              <label className="flex items-center gap-2 cursor-pointer select-none text-xs">
                <div className="relative flex items-center">
                  <input
                    type="checkbox"
                    className="sr-only"
                    checked={Boolean(m.movinmotion_contrat_ok)}
                ...
                <span
                  ...
                  style={{
                    color: m.movinmotion_contrat_ok ? 'var(--purple)' : 'rgba(156,95,253,.6)',
                  }}
                >
                  MovinMotion
                </span>
              ...
              <a
                ...
                onMouseLeave={(e) => {
                  e.currentTarget.style.color = 'rgba(156,95,253,.5)'
                  e.currentTarget.style.background = 'transparent'
                }}
              >
                <ExternalLink className="w-3 h-3" />
...
function KpiCard({ label, value, sub, color = 'blue' }) {
  const fg =
    { blue: 'var(--blue)', green: 'var(--green)', purple: 'var(--purple)', amber: 'var(--amber)' }[
      color
    ] || 'var(--blue)'
  return (
    <div
    ...
    >
      ...
      <p className="text-base font-bold leading-tight" style={{ color: fg }}>
        {value}
      </p>
      {sub && (
