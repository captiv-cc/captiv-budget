...
export default function KpiBar({ global: g, refDevis }) {
  const ecartColor =
    g.ecartCout > 0.01 ? 'var(--red)' : g.ecartCout < -0.01 ? 'var(--green)' : 'var(--txt-3)'
  const margeColor =
    g.margeReelle < 0 ? 'var(--red)' : g.deltaMarge < -0.01 ? 'var(--amber)' : 'var(--green)'
    ...
    >
      ...
      <div className="flex items-stretch py-3">
        <Block label="Vente HT">
          <p className="font-bold tabular-nums text-lg" style={{ color: 'var(--blue)' }}>
            {fmtEur(g.venteHT)}
          </p>
        ...
        <Block label="Coût  prévu → réel" flex={1.4}>
          <div className="flex items-baseline gap-1.5">
            <span className="font-semibold tabular-nums text-sm" style={{ color: 'var(--amber)' }}>
              {fmtEur(g.coutPrevu)}
            </span>
            <span style={{ color: 'var(--txt-3)', fontSize: 11 }}>→</span>
            <span
              className="font-bold tabular-nums text-lg"
              style={{ color: g.ecartCout > 0.01 ? 'var(--red)' : 'var(--green)' }}
            >
              {fmtEur(g.coutReelProjete)}
            </span>
          </div>
          <p className="text-[9px]" style={{ color: ecartColor }}>
            {g.ecartCout > 0 ? '+' : ''}
            {fmtEur(g.ecartCout)} ({g.ecartCout > 0 ? '+' : ''}
            {ecartPct.toFixed(1)} %){' '}
            {g.ecartCout > 0.01 ? '· dépassement' : g.ecartCout < -0.01 ? '· économie' : '· =prévu'}
          </p>
        </Block>
        ...
        <Block label="Marge  prévue → réelle" flex={1.4}>
          <div className="flex items-baseline gap-1.5">
            <span className="font-semibold tabular-nums text-sm" style={{ color: 'var(--purple)' }}>
              {fmtEur(g.margePrevue)}
            </span>
            <span style={{ color: 'var(--txt-3)', fontSize: 11 }}>→</span>
            <span className="font-bold tabular-nums text-lg" style={{ color: margeColor }}>
              {fmtEur(g.margeReelle)}
            </span>
          </div>
          <p className="text-[9px]" style={{ color: margeColor }}>
            {margePct.toFixed(1)} % du CA · {g.deltaMarge > 0 ? '+' : ''}
            {fmtEur(g.deltaMarge)} vs prévu
          </p>
        </Block>
      </div>
      ...
      <div
        className="flex items-center px-4 py-1.5 gap-4 text-[10px]"
      ...
      >
        <div className="flex items-center gap-2 flex-1">
          ...
          >
            <div
              style={{
                width: `${Math.min(100, g.avancement)}%`,
                height: '100%',
                background:
                  g.avancement > 100.5
                    ? 'var(--red)'
          ...
          >
            {g.avancement.toFixed(0)} %
          </span>
          <span style={{ opacity: 0.7 }}>
            · {g.nbLignesSaisies}/{g.nbLignes} lignes confirmées
          </span>
        </div>
        <span>·</span>
        <span>
        ...
        <span>
          ...
          <span
            className="tabular-nums font-semibold"
            style={{ color: g.resteRegler > 0 ? 'var(--amber)' : 'var(--green)' }}
          >
            {fmtEur(g.resteRegler)}
          </span>
        </span>
      </div>
      ...
      >
        ...
        <span>
          Collectée{' '}
          <span className="tabular-nums font-semibold" style={{ color: 'var(--blue)' }}>
            {fmtEur(g.tvaCollectee)}
          </span>
        </span>
        <span>·</span>
        <span>
