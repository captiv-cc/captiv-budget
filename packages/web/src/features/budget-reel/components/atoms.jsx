...
export function BlocTotal({ label, value, color = 'default' }) {
  ...
  return (
    <span>
      ...
      <span className="font-bold tabular-nums" style={{ color: C[color] || C.default }}>
        {value}
      </span>
    </span>
  )
...
export function StatusToggle({ isEstime, _valide, paye, onConfirmAtPrevu, onTogglePaye }) {
  let _state, color, title, fill, Icon, onClick
  if (paye) {
    _state = 'paye'
    color = 'var(--green)'
    fill = true
  ...
  return (
    <button
      ...
      style={{
        ...
        border: `1.5px solid ${checked ? c : 'var(--brd)'}`,
      }}
    >
...
export function Th({ children, left, right, _center, style }) {
  return (
    <th
      className="px-3 py-2 font-semibold uppercase tracking-wide whitespace-nowrap"
      style={{
        color: 'var(--txt-3)',
        fontSize: 10,
