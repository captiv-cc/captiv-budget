...
| LIV-14 ✅ | Bulk edit (sélection multiple) | M | Checkbox custom 14/16px (`Checkbox.jsx`) sur chaque ligne + header bloc (état tri none/partial/all). State centralisé `selectedIds: Set` dans `LivrablesTab` (cross-blocs). Shift+click = range. `BulkActionBar.jsx` bottom-fixed avec 7 actions : Statut (popover top), Monteur, Date, Format (popover top), Frame, Drive, Supprimer (toast undo unique 5 s). `PopoverFloat` étendu avec prop `placement: 'top'` pour ouvrir vers le haut quand l'ancre est en bas du viewport. |
| LIV-15 ✅ | Filtres (monteur, statut, Mes livrables) | S | `LivrablesFilterBar.jsx` barre horizontale sous le header (4 multi-select : Statut / Monteur / Format / Bloc + 2 toggles : En retard / Mes livrables). State synchronisé via `useSearchParams` → URL params persistants & partageables (pas de localStorage). Helpers purs `filterLivrables(livrables, filters, ctx)` + `hasActiveFilter(filters)` dans `livrablesHelpers.js`. Filtre "Mes livrables" branché sur `assignee_profile_id === userId`, complété par un nouveau composant `MonteurInput.jsx` (autocomplete profile org + fallback texte libre, propre commit `{ profileId, external }` qui patch les 2 champs en un appel) — supplante l'ancien input texte libre des LIV-7. Lib `profiles.js` (`listOrgProfiles({ orgId })` + `indexProfilesById`) chargée au mount du tab. Tests : 14 cas filtres + listMonteurs migré sur `full_name`. |
| LIV-16 ✅ | Compteurs retard + prochain (header LIV) | S | Compteurs du header refactorés en chips cliquables (boutons-filtres) : Total = eraser (efface tous les filtres), Actifs = preset statuts {brief, en_cours, a_valider, valide}, En retard = toggle `enRetard`, Livrés = preset statuts {livre}. État actif = bg teinté + bordure colorée + texte coloré. Détection preset actif = égalité stricte des Sets. Nouveau chip `ProchainChip` (orange, format `numero nom · dateRelative`) = livrable non terminé le plus proche dans le futur. Click chip = scroll-to + flash highlight 1,8 s sur la ligne (au lieu d'ouvrir le drawer — voir le livrable EN CONTEXTE dans son bloc). En complément, marqueur permanent (pastille orange à gauche du numero) sur la ligne du "prochain" pour identification visuelle sans interaction. data-livrable-id sur la `<tr>` + `<div>` mobile pour le scrollIntoView. Compteurs basés sur la liste GLOBALE du projet (pas filtrée — sinon le filtre se mord la queue). Helper pur `formatDateRelative(dateISO, now)` → `Aujourd'hui` / `Demain` / `Hier` / `dans X j` / `dans X sem.` / `le JJ/MM[/AAAA]`. Tests : 7 nouveaux cas formatDateRelative. |
| LIV-17 ✅ | Widget accueil projet | S | `LivrablesProjectWidget.jsx` self-contained (fait son propre fetch via `useLivrables(projectId)`). Intégré dans `ProjetTab.jsx` en remplacement du placeholder legacy "Aucun livrable défini" + de la `SectionCard` qui listait `livrables_json` (champ obsolète depuis LIV-1). Contenu : header titre + count + lien "Voir tout →", bandeau orange "Prochain livrable" (numero + nom + dateRelative + monteur avatar) caché si aucun à venir, 3 mini-compteurs (Total / En retard / Livrés), liste des **3 prochains** livrables non terminés (n° + nom + dateRelative + monteur). Empty state si 0 livrable avec CTA "Ajouter un livrable". Click sur n'importe quel item → navigation vers `/projets/:id/livrables`. Masqué si `!canSeeLivrables`. Le legacy `livrables_json` reste en DB et reste éditable depuis le mode édition de `ProjetTab` (pas touché — out of scope LIV-17, à nettoyer plus tard). Cible aussi LIV-18 (page globale) avec une éventuelle variante mode='compact' réutilisable. |
| LIV-18 ✅ | Widget index global | M | Helper `listUpcomingLivrables({ orgId, daysAhead, limit })` ajouté à `src/lib/livrables.js` — query Supabase avec join `projects!inner` (filtre `org_id`), inclut tous les livrables non terminés (statuts `brief/en_cours/a_valider/valide`) avec `date_livraison <= today + daysAhead` (les en retard inclus volontairement). RLS gère l'accessibilité user. Composant `LivrablesGlobalWidget.jsx` self-contained dans `src/features/livrables/components/`, intégré dans `HomePage.jsx` à la place de la section "Deadlines à venir" (qui faisait un fetch legacy sur `planning_items`). Helper local `formatDeadlineShort` compact (Auj./Demain/Hier/J+N/JJ/MM) car la sidebar a peu de place. Click ligne → `/projets/:id/livrables`. Bonus : fix bug du compteur StatCard "Livrables en cours" (excluait `valide` au lieu de `archive`) → maintenant utilise `not in ("livre","archive")` cohérent avec `LIVRABLE_STATUTS_TERMINES`. La carte "Deadlines cette semaine" (visible uniquement aux non-finance) supprimée — la liste à droite couvre déjà le besoin. Le `daysUntil` helper legacy retiré. |
| LIV-19 ✅ | Lien devis Niveau 1 (pointeur) | S | **Design métier** : un bloc = un thème commercial = un lot. Le pointeur vit sur le **bloc**, pas sur le livrable (règle : tous les livrables d'un même bloc partagent le même lot). **Migration** `20260430_liv19_block_devis_lot.sql` ajoute `livrable_blocks.devis_lot_id` (FK vers `devis_lots`, ON DELETE SET NULL) + index partiel. Le legacy `livrables.devis_lot_id` (LIV-1) est conservé en DB pour rétro-compat mais n'est plus utilisé. **A — UI lien** : sous-menu "Lier à un lot…" dans le menu ⋯ du `LivrableBlockCard` (header du bloc), `PopoverFloat` ancré au bouton menu. `BlockLotPickerMenu` liste les lots non archivés du projet (option "Aucun lot (générique)" en tête, checkmark sur le lot courant). Helper `listProjectLots(projectId)` dans `livrables.js`. `BLOCK_EDITABLE_FIELDS` étendu avec `devis_lot_id`. Lots chargés une fois au mount du tab via `LivrablesTab`, plumb à travers BlockList → BlockCard. **B — PDF devis** : `exportDevisPDF` charge en parallèle `listLivrablesForDevisPdf(projectId, devis.lot_id)` qui (1) liste tous les blocs du projet, garde ceux avec `block.devis_lot_id === lotId OR NULL`, (2) liste les livrables de ces blocs (exclut `archive` + soft-deleted), (3) trie par `numero` en mode naturel via `localeCompare(numeric: true)` — gère bien `A1 < A2 < A10`. Sans-numero à la fin. Format ligne : `{numero} \| {nom} \| {format} \| {duree}` (max 8 livrables). Si 0 livrable filtré → section "Livrable(s) :" complètement cachée. Le legacy `livrables_json` n'est plus lu côté PDF. |
| LIV-20 ✅ | Soft delete + corbeille | M | **Migration** `20260430_liv20_soft_delete_versions_etapes.sql` ajoute `deleted_at` sur `livrable_versions` et `livrable_etapes` (les blocs/livrables l'avaient déjà depuis LIV-1) + index partiels. **Lib refacto** : `deleteVersion` / `deleteEtape` basculent en soft delete (UPDATE deleted_at), `fetchVersions` / `fetchEtapes` filtrent désormais sur `deleted_at IS NULL`. Nouveaux helpers : `restoreVersion`, `purgeVersion`, `restoreEtape` (avec recréation event miroir si is_event=true), `purgeEtape`, `purgeBlock`, `purgeLivrable`, `fetchTrash(projectId, daysAhead=30)` qui aggrège blocs / livrables / versions / étapes soft-deleted dans la fenêtre. **Hook** : 7 nouvelles actions exposées (purge*, restoreVersion/Etape, fetchTrash). **UI** `LivrablesTrashDrawer.jsx` : drawer side-right (480px) avec 4 sections (Blocs / Livrables / Versions / Étapes), chaque ligne a `Restaurer` (bg vert) + `Supprimer définitivement` (bg rouge, confirm avant). Empty state si rien depuis 30 jours. Bouton icône Trash dans le header LivrablesTab à côté de "Nouveau bloc" (visible si canEdit). Pas de "tout restaurer" (V1 — restaurer un par un). |
| LIV-21 ✅ | Tests + validation finale | M | **Couverture unitaire** complète des helpers purs (`livrablesHelpers.test.js`) : sortBySortOrder, group/index helpers, isLivrableEnRetard, computeCompteurs (total/actifs/enRetard/livres/valides/prochain), listMonteurs (avec full_name), nextLivrableNumero, pickAllowed, parseDuree, dureeToSeconds, monteurAvatar, computeLivrableStatutFromVersions, formatDateRelative (8 cas), filterLivrables (10 cas), hasActiveFilter (4 cas) — **80+ tests**. **ESLint global** sur tout le scope LIV (lib, hook, 23 composants, page, tests, sync) → 0 warning lié au chantier. **Smoke test** Node : 27 cas couvrant tous les helpers exportés (formatDateRelative, computeCompteurs.prochain, filterLivrables, listMonteurs, dureeToSeconds, parseDuree, computeLivrableStatutFromVersions, hasActiveFilter, pickAllowed, nextLivrableNumero) — 27/27 pass. **Volumétrie finale Vague 1** : 23 composants livrables, ~13 250 lignes (lib + hook + composants + tests + sync), 8 migrations SQL, 19 tickets fonctionnels livrés (LIV-1 à LIV-21, LIV-10 skip volontaire). |

---

## ✅ Vague 1 LIV — Bouclée (2026-04-30)

**Tickets livrés** : 19/19 (LIV-1 à LIV-21, sauf LIV-10 skipped — phases projet jugées hors scope LIV).

**Volumétrie** : ~13 250 lignes de code, 23 composants React, 8 migrations SQL, ~80 tests unitaires, 0 warning ESLint.

**Périmètre couvert** :
- CRUD blocs + livrables + versions + étapes (LIV-3 à LIV-9)
- Drag & drop intra-bloc + entre blocs (LIV-6, LIV-11)
- Duplication livrable + cross-project (LIV-12, LIV-13)
- Bulk edit (LIV-14)
- Filtres barre horizontale + autocomplete monteur (LIV-15)
- Compteurs interactifs + chip "Prochain" (LIV-16)
- Widgets accueil projet + index global (LIV-17, LIV-18)
- Lien lot devis sur les blocs + livrables dans PDF (LIV-19)
- Soft delete + corbeille avec restauration et purge (LIV-20)

**Vague 2 (à venir)** : pipeline swimlanes Gantt, PDF dédiés (V and B Fest + Mandatory Wow), Niveau 3 devis (verrouillage facturation).
