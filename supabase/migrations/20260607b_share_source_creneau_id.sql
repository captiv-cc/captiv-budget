-- ============================================================================
-- Migration : DÉROULÉ — Exposer source_creneau_id dans les RPCs share publiques.
-- Date      : 2026-06-07 (B — follow-up de 20260607_share_creneau_alertes.sql)
-- Contexte  : Suite à l'ajout des alertes côté share, on s'est aperçu qu'un
--             créneau ENFANT (lien soft link via source_creneau_id) sans
--             alerte propre ne pouvait pas hériter visuellement de l'alerte
--             de son créneau source. Cas typique :
--               - "Hamza @ scène Le Château" a alerte_text = '3P CRASHS'
--               - "Hamza @ cadreur Hugo Martin" (enfant) n'a pas d'alerte
--               → le cadreur ne voit pas l'info "3P CRASHS" sur sa mission
--
--             Pour permettre cet héritage côté client, on expose maintenant
--             c.source_creneau_id dans le payload des RPCs share. Le client
--             peut alors lookup le parent et hériter de l'alerte (helper
--             effectiveAlerte() dans lib/deroule.js).
--
-- Périmètre :
--   1. CREATE OR REPLACE share_deroule_fetch (ajoute source_creneau_id)
--   2. CREATE OR REPLACE share_projet_deroule_fetch (idem)
--
-- Note : Migration séparée de 20260607_share_creneau_alertes.sql pour le
-- cas où cette dernière aurait déjà été appliquée sans le champ
-- source_creneau_id. Supabase ne re-run pas une migration déjà marquée
-- "applied" si on modifie le fichier.
--
-- Pas sensible (juste un UUID interne), pas gated par show_sensitive.
    ...
    'creneaux', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          -- Nouveau : id du créneau source si soft link. Permet à l'UI
          -- de retrouver le parent pour l'héritage d'alerte.
...
-- ── 2. share_projet_deroule_fetch (portail projet) ─────────────────────────
