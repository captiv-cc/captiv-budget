-- ============================================================================
-- Migration : DÉROULÉ — Exposer les alertes (alerte_text / alerte_niveau)
--             dans les RPCs share publiques.
-- Date      : 2026-06-07
-- Contexte  : Les alertes (FEST-5.4 — info/important sur créneaux) sont
--             présentes en BDD mais les RPCs publiques `share_deroule_fetch`
--             et `share_projet_deroule_fetch` ne les exposaient pas dans
--             le payload. Du coup côté page partagée (popover détail,
--             timeline, vue cadreur, exports PDF/PNG) les alertes étaient
--             invisibles malgré l'existence du contenu côté admin.
--
-- Périmètre :
--   1. CREATE OR REPLACE share_deroule_fetch (ajoute alerte_text +
--      alerte_niveau au bloc 'creneaux')
--   2. CREATE OR REPLACE share_projet_deroule_fetch (idem)
--
-- Note : les alertes sont considérées non-sensibles (info de prod utile au
-- destinataire externe), donc PAS gated par show_sensitive.
--
    ...
    'creneaux', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          -- Alertes (FEST-5.4) : exposées car info utile pour le destinataire
    ...
    'creneaux', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          'statut',          c.statut,
          'sort_order',      c.sort_order,
          -- Alertes (FEST-5.4) idem que share_deroule_fetch ci-dessus.
          'alerte_text',     c.alerte_text,
          'alerte_niveau',   c.alerte_niveau,
          'notes',           CASE WHEN v_show_sensitive THEN c.notes ELSE NULL END,
          'member_ids',      COALESCE((
