-- ============================================================================
-- Migration : DÉROULÉ — Exposer project.creneau_types (types custom V2) dans
--             les RPCs share publiques.
-- Date      : 2026-06-08 (B — follow-up de 20260608_creneau_types_v2.sql)
-- Contexte  : Sprint Festival types V2 a introduit projects.creneau_types
--             (JSONB) pour permettre des types personnalisés par projet
--             (ex : "Pyrotechnie", "Maquillage", "Brief sécurité"). Côté
--             admin, lib/creneauTypes.js + UI gèrent le mapping :
--               creneau.type  → projects.creneau_types[].key → couleur+libellé
--
--             Sans cette migration, les destinataires d'un lien de partage
--             public ne voient pas les types custom : les blocs concernés
--             rendent en gris fallback (CRENEAU_TYPE_COLORS.autre) et la
--             vue cadreur affiche la clé brute ("pyrotechnie") au lieu du
--             libellé propre ("Pyrotechnie").
--
--             Solution : exposer p.creneau_types dans le payload 'project'
--             des deux RPCs publiques. Le frontend share peut alors utiliser
--             getProjectCreneauTypes(project) comme dans l'admin pour
--             résoudre couleurs + libellés.
--
-- Périmètre :
--   1. CREATE OR REPLACE share_deroule_fetch (ajoute creneau_types)
--   2. CREATE OR REPLACE share_projet_deroule_fetch (idem)
--
-- Sensibilité : aucune. Les types custom sont des étiquettes / couleurs
-- d'organisation interne, pas de donnée sensible. Pas gated par
-- show_sensitive.
--
