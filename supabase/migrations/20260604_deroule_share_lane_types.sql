-- ============================================================================
-- Migration : DÉROULÉ FESTIVAL Sprint 1 — RPCs share avec lanes typées
-- Date      : 2026-06-04
-- Contexte  : FEST-5 — la page share doit pouvoir basculer en "Vue Cadreur".
--             Pour ça il faut que le payload retourne les nouveaux champs
--             des lanes (type / membre_id / couleur — FEST-1).
--
-- Périmètre :
--   1. CREATE OR REPLACE share_deroule_fetch (lien dédié /share/deroule/:token)
--   2. CREATE OR REPLACE share_projet_deroule_fetch (sous-page portail
--      /share/projet/:token/deroule)
--
-- Seul le bloc 'lanes' du payload change : ajout de type, membre_id, couleur.
-- Le reste du payload est strictement identique à la dernière version
-- (20260513_deroule_share_name_priority.sql).
--
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- FEST-5 : ajout type / membre_id / couleur sur les lanes
    'lanes', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          'id',         l.id,
